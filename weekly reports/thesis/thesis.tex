...
\[
det(J_{N_1})=e^{-\gamma\tau/2}, det(J_{N_2})=e^{-\gamma(T-\tau)/2}
\]
...
involving only one collision per cycle (termed $PnC1$ orbits)\footnote{$n$ 
signifies the periodicity of the orbit} with the hard wall.  But this method does not 
...
Grebogi et al. \cite{grebogi-transient} holds true for impacting systems or 
...
& &+\colv{0}{-2v_h(\tau+\delta t)-2v_p(\tau+\delta t)} \colv{x(T)}{v(T)}\\
...
Using this Poincaré map, we can very easily plot the bifurcation diagrams.  
...
Fig \ref{fig-bif-int} and Fig \ref{fig-bif-nonint} clearly demonstrates the 
curious effect on grazing bifurcations of making $n=\frac{2\omega_g}{\omega}$ 
an integer. In the non-integral case, as 
...
We see that in case of both integral $n$ and non-integral $n$, a little bit 
after grazing periodic orbits emerged.  We decided to take a look at the phase 
...
\subsection{Numerical computation of the fixed points}
...
quite larger than the grazing condition ($0.3903$, to be exact), all the 
...
We had talked about the concept of impact maps in subsection 
{subsec-impact-map}. Now we will apply it in order to tackle the problem of 
